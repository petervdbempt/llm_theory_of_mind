...
def run_tournament(args, games=6, use_seeds = True):
    seeds = [3, 18, 16, 32, 88, 0, 12, 21, 29, 35] 
    
    base_log_dir = Path("tournament_logs_TOM")
    base_log_dir.mkdir(exist_ok=True)

    agent_types = ["TOM", "CLAUDE", "GEMINI"]
...
def find_interesting_seeds(num_games: int = 100, top_k: int = 10, seed_start: int = 0):
    class DummyLogger:
        def log(self, msg): 
            pass
    ...
    for i in range(num_games):
        ...
        board_map, player_states = ColoredTrails.generate_random_game(seed=seed)
        game = ColoredTrails(board_map, player_states)
        ...
        offers_made = result
        print(offers_made)
        total_offers = offers_made.get("p1", 0) + offers_made.get("p2", 0)

        interesting_seeds.append({
            "seed": seed,
            "offers_p1": offers_made.get("p1", 0),
            "offers_p2": offers_made.get("p2", 0),
            "total_offers": total_offers,
        })

        print(f"Game {i+1}/{num_games} | Seed {seed} | Offers: {total_offers}")

    interesting_seeds.sort(key=lambda x: x["total_offers"], reverse=True)

    print(f"\nTop {top_k} most active (offer-heavy) games:")
    for s in interesting_seeds[:top_k]:
        print(f"Seed {s['seed']} | Total offers: {s['total_offers']} (P1={s['offers_p1']}, P2={s['offers_p2']})")

    return interesting_seeds[:top_k]
