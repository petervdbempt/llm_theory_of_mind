...
import matplotlib.patheffects as pe  # NEW: Correct import for path effects
import numpy as np
...
# New: Matplotlib color mapping configuration
# 1. Define value mapping for the board strings
COLOR_TO_VALUE = {"WHT": 0, "GRY": 1, "BLK": 2}
...
def plot_game_state(game: ColoredTrails):
    ...
    # Convert the string board (BLK, GRY, WHT) into a numerical matrix (0, 1, 2)
    ...
    # Start Position (S)
    ax.text(s_pos[1], s_pos[0], 'S', ha='center', va='center', fontsize=20, color='gold',
            path_effects=[
                pe.withStroke(linewidth=1, foreground='black')  # FIX: Corrected to pe.withStroke
            ])

    # P1 Goal (1)
    ax.text(g1_pos[1], g1_pos[0], '1️', ha='center', va='center', fontsize=20, color='gold',
            path_effects=[
                pe.withStroke(linewidth=1, foreground='black')  # FIX: Corrected to pe.withStroke
            ])
    ...
    if g2_pos != g1_pos:
        ax.text(g2_pos[1], g2_pos[0], '2️', ha='center', va='center', fontsize=20, color='gold',
            path_effects=[
                pe.withStroke(linewidth=1, foreground='black')  # FIX: Corrected to pe.withStroke
            ])
    ...
    plt.tight_layout(rect=[0, 0, 1, 0.95])  # Adjust layout to make room for the info box
...
def run_game_simulation(game: ColoredTrails):
    ...
    print("=" * 60)
    print("Initial Chip Distribution (Player Hand):")
    ...
    # We use rounds_played to track the negotiation penalty
    rounds_played = MAX_NEGOTIATION_ROUNDS

    for round_num in range(1, MAX_NEGOTIATION_ROUNDS + 1):
        proposer_id = current_proposer
        responder_id = 'p2' if current_proposer == 'p1' else 'p1'
        ...
        print(f"\n--- Round {round_num} (Proposer: {proposer_id.upper()}) ---")

        if proposer_give == "Pass":
            print(f"  -> {proposer_id.upper()} passes. Negotiation ends.")
            rounds_played = round_num  # Only penalize for rounds actually used
            break

        print(
            f"  -> PROPOSAL: {proposer_id.upper()} offers to GIVE: {proposer_give} for RECEIVING: {proposer_receive} from {responder_id.upper()}")
        ...
        # Switch the proposer for the next round
        current_proposer = responder_id
    ...
    # Apply negotiation penalty
    penalty = rounds_played * PENALTY_PER_ROUND
    p1_final_score = p1_max_score - penalty
    p2_final_score = p2_max_score - penalty

    final_scores['p1'] = p1_final_score
    final_scores['p2'] = p2_final_score

    print("\n--- FINAL RESULTS ---")
    print(f"Total Negotiation Rounds: {rounds_played} (Penalty: {penalty} points each)")
    print(f"P1 Final Score: {p1_final_score} (Max Path Score: {p1_max_score})")
